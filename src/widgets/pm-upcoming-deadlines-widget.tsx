...
export function PmUpcomingDeadlinesWidget() {
        ...
        <CardContent>
          <div className="space-y-4">
            {deadlines.length > 0 ? deadlines.map((dl, i) => (
              <div key={i} className={`flex flex-col p-3 rounded-lg border ${dl.isUrgent ? 'border-orange-500/50 bg-orange-500/5' : 'border-border/50 bg-muted/10'}`}>
                <div className="flex items-start justify-between mb-1">
                  <span className={`font-medium text-sm line-clamp-1 mr-2 ${dl.isUrgent ? 'text-orange-500 font-semibold' : ''}`}>{dl.event}</span>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${dl.status === 'Critical' ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground'}`}>
                    {dl.status}
                  </span>
                </div>
                <span className={`text-xs ${dl.isUrgent ? 'text-orange-500/90 font-medium' : 'text-muted-foreground'}`}>Due {dl.date}</span>
              </div>
            )) : (
